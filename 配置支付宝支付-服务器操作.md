# 配置支付宝支付 - 服务器操作

## 📋 需要的信息

在配置之前，请确保你已经准备好以下信息：

1. **支付宝 AppID**（应用ID）
2. **应用私钥（Private Key）**
3. **支付宝公钥（Public Key）**

如果没有，需要先在支付宝开放平台创建应用。

---

## 🚀 配置步骤

### 步骤 1：编辑 .env 文件

在服务器上执行（阿里云控制台的对话框里）：

```bash
cd /var/www/geshixiugai
vi .env
```

### 步骤 2：在 vi 编辑器中添加支付宝配置

1. **跳转到文件末尾**：按 `G`（大写）

2. **进入编辑模式**：按 `i`

3. **添加支付宝配置**：

```bash
# 支付宝支付配置
ALIPAY_APP_ID=你的支付宝AppID
ALIPAY_PRIVATE_KEY=你的应用私钥（完整内容）
ALIPAY_PUBLIC_KEY=支付宝公钥（完整内容）
ALIPAY_SIGN_TYPE=RSA2
ALIPAY_GATEWAY=https://openapi.alipay.com/gateway.do
BASE_URL=https://geshixiugai.cn
```

**重要提示**：
- `ALIPAY_PRIVATE_KEY`：如果包含 `-----BEGIN RSA PRIVATE KEY-----` 和 `-----END RSA PRIVATE KEY-----`，需要完整复制包括这些标记
- `ALIPAY_PUBLIC_KEY`：支付宝公钥，可能没有 BEGIN/END 标记，直接粘贴即可
- 私钥和公钥内容较长，复制时请确保完整复制，不要遗漏任何字符

4. **保存退出**：
   - 按 `Esc` 退出编辑模式
   - 输入 `:wq`
   - 按回车

### 步骤 3：确保文件权限正确

```bash
# 确保 .env 文件权限正确（服务需要读取）
sudo chown nginx:nginx /var/www/geshixiugai/.env
sudo chmod 600 /var/www/geshixiugai/.env
```

### 步骤 4：重启服务

```bash
sudo systemctl restart geshixiugai
sudo systemctl status geshixiugai
```

### 步骤 5：查看日志确认配置

```bash
# 查看日志，确认支付宝配置加载成功
sudo tail -n 50 /var/log/geshixiugai/error.log | grep -i alipay
```

应该看到：`[AlipayService] 支付宝客户端初始化成功`

---

## ✅ 验证配置

### 方法 1：访问调试端点

在浏览器中访问：
```
https://geshixiugai.cn/payments/debug/config
```

应该返回 JSON，包含支付宝配置信息：
```json
{
  "alipay": {
    "app_id_exists": true,
    "app_id_preview": "2021****",
    "private_key_exists": true,
    "public_key_exists": true,
    "configured": true
  }
}
```

### 方法 2：测试支付流程

1. 访问：https://geshixiugai.cn
2. 上传模板和文档
3. 进入支付页面
4. 选择"支付宝支付"
5. 点击"立即支付"
6. 应该跳转到支付宝支付页面

---

## 📋 完整操作流程

### 1. 编辑 .env 文件

```bash
cd /var/www/geshixiugai
vi .env
# 添加支付宝配置，保存退出
```

### 2. 设置文件权限

```bash
sudo chown nginx:nginx /var/www/geshixiugai/.env
sudo chmod 600 /var/www/geshixiugai/.env
```

### 3. 重启服务

```bash
sudo systemctl restart geshixiugai
sudo systemctl status geshixiugai
```

### 4. 验证配置

- 访问：https://geshixiugai.cn/payments/debug/config
- 测试支付流程

---

## ⚠️ 重要提示

### 1. 私钥格式

- 如果私钥包含 `-----BEGIN RSA PRIVATE KEY-----` 和 `-----END RSA PRIVATE KEY-----`，需要完整复制包括这些标记
- 如果私钥是纯 base64 字符串，直接粘贴即可，系统会自动格式化

### 2. 公钥格式

- 支付宝公钥可能没有 BEGIN/END 标记，直接粘贴即可
- 系统会自动添加标记

### 3. 回调地址

确保在支付宝开放平台配置的回调地址是：
- **服务器异步通知页面路径**：`https://geshixiugai.cn/payments/alipay/notify`

---

## 🔍 故障排查

### 问题 1：支付订单创建失败

```bash
# 查看服务日志
sudo tail -n 100 /var/log/geshixiugai/error.log | grep -i alipay
```

### 问题 2：配置未生效

```bash
# 检查 .env 文件
cat /var/www/geshixiugai/.env | grep ALIPAY

# 检查文件权限
ls -la /var/www/geshixiugai/.env
```

### 问题 3：服务启动失败

```bash
# 查看服务状态
sudo systemctl status geshixiugai

# 查看详细日志
sudo journalctl -u geshixiugai -n 50 --no-pager
```

---

**现在开始配置支付宝支付！** 💰


