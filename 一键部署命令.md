# 一键部署命令 - geshixiugai.cn

## 🚀 快速部署（复制粘贴即可）

### 步骤 1：连接到服务器

```bash
ssh root@你的服务器公网IP
```

### 步骤 2：执行部署命令

```bash
# 创建项目目录
mkdir -p /var/www && cd /var/www

# 克隆代码
git clone https://github.com/jet20002025-hash/geshixiugai.git geshixiugai

# 进入项目目录
cd geshixiugai

# 给部署脚本添加执行权限
chmod +x deploy_aliyun.sh

# 运行部署脚本
sudo ./deploy_aliyun.sh
```

### 步骤 3：配置环境变量

部署脚本运行完成后，会提示你配置 `.env` 文件：

```bash
cd /var/www/geshixiugai

# 复制环境变量模板
cp env.example .env

# 编辑环境变量（填入你的配置）
nano .env
```

在 `.env` 文件中填入你的配置信息（存储、支付等），保存退出。

### 步骤 4：重启服务

```bash
# 重启服务使环境变量生效
sudo systemctl restart geshixiugai

# 检查服务状态
sudo systemctl status geshixiugai
```

### 步骤 5：配置域名解析

1. 登录阿里云 DNS 控制台：https://dns.console.aliyun.com/
2. 找到域名 `geshixiugai.cn`
3. 添加 A 记录：
   - **主机记录**：@，**记录值**：你的服务器公网 IP
   - **主机记录**：www，**记录值**：你的服务器公网 IP

### 步骤 6：配置 SSL 证书

```bash
# 安装 Certbot
sudo apt install -y certbot python3-certbot-nginx

# 申请证书（会自动配置 Nginx）
sudo certbot --nginx -d geshixiugai.cn -d www.geshixiugai.cn
```

### 步骤 7：测试访问

在浏览器中访问：
- `https://geshixiugai.cn`
- `https://geshixiugai.cn/web`
- `https://geshixiugai.cn/docs`

---

## 📋 完整命令（一次性复制）

```bash
# 1. 克隆代码
mkdir -p /var/www && cd /var/www
git clone https://github.com/jet20002025-hash/geshixiugai.git geshixiugai
cd geshixiugai

# 2. 运行部署脚本
chmod +x deploy_aliyun.sh
sudo ./deploy_aliyun.sh

# 3. 配置环境变量（脚本运行完成后）
cp env.example .env
nano .env  # 填入你的配置，保存退出

# 4. 重启服务
sudo systemctl restart geshixiugai
sudo systemctl status geshixiugai

# 5. 配置 SSL（域名解析完成后）
sudo apt install -y certbot python3-certbot-nginx
sudo certbot --nginx -d geshixiugai.cn -d www.geshixiugai.cn
```

---

## 🔍 验证部署

### 检查服务状态

```bash
# 检查服务是否运行
sudo systemctl status geshixiugai

# 检查端口是否监听
sudo netstat -tlnp | grep 8000

# 测试本地访问
curl http://localhost:8000
```

### 查看日志

```bash
# 查看服务日志
sudo journalctl -u geshixiugai -f

# 查看 Nginx 错误日志
sudo tail -f /var/log/nginx/geshixiugai_error.log
```

---

## ⚠️ 注意事项

1. **环境变量配置**：确保 `.env` 文件中配置了正确的存储信息（Supabase/R2/B2）
2. **域名解析**：配置 DNS 后需要等待几分钟到几小时生效
3. **SSL 证书**：Certbot 会自动配置，证书每 90 天自动续期
4. **备案号**：记得在网站底部添加备案号显示

---

## 📞 需要帮助？

- 查看详细文档：`阿里云快速部署指南.md`
- 查看完整指南：`阿里云迁移完整指南.md`
- 查看准备清单：`部署准备清单.md`

---

**部署完成后，你的网站将在 `https://geshixiugai.cn` 正常运行！** 🎉








