# 服务器部署命令 - geshixiugai.cn

## 🖥️ 服务器信息

- **公网 IP**：121.199.49.1
- **域名**：geshixiugai.cn
- **GitHub 仓库**：https://github.com/jet20002025-hash/geshixiugai

---

## 🚀 快速部署（复制粘贴执行）

### 步骤 1：连接到服务器

```bash
ssh root@121.199.49.1
```

### 步骤 2：执行部署命令（一次性复制）

```bash
# 创建项目目录并克隆代码
mkdir -p /var/www && cd /var/www
git clone https://github.com/jet20002025-hash/geshixiugai.git geshixiugai
cd geshixiugai

# 运行部署脚本
chmod +x deploy_aliyun.sh
sudo ./deploy_aliyun.sh
```

### 步骤 3：配置环境变量

部署脚本运行完成后，配置 `.env` 文件：

```bash
cd /var/www/geshixiugai
cp env.example .env
nano .env
```

在 `.env` 文件中填入你的配置：
- Supabase Storage 配置（或 R2/B2）
- 支付配置（如果有）

保存退出（`Ctrl+O`，`Enter`，`Ctrl+X`）。

### 步骤 4：重启服务

```bash
sudo systemctl restart geshixiugai
sudo systemctl status geshixiugai
```

### 步骤 5：配置域名解析

1. 登录阿里云 DNS 控制台：https://dns.console.aliyun.com/
2. 找到域名 `geshixiugai.cn`
3. 添加 A 记录：
   - **主机记录**：@
   - **记录类型**：A
   - **记录值**：121.199.49.1
   - **TTL**：600
4. 添加 www 记录：
   - **主机记录**：www
   - **记录类型**：A
   - **记录值**：121.199.49.1
   - **TTL**：600

### 步骤 6：配置 SSL 证书

等待域名解析生效后（通常几分钟），执行：

```bash
# 安装 Certbot
sudo apt install -y certbot python3-certbot-nginx

# 申请证书
sudo certbot --nginx -d geshixiugai.cn -d www.geshixiugai.cn
```

### 步骤 7：测试访问

在浏览器中访问：
- `http://121.199.49.1`（应该重定向到 HTTPS）
- `https://geshixiugai.cn`（域名解析生效后）
- `https://geshixiugai.cn/web`（前端页面）
- `https://geshixiugai.cn/docs`（API 文档）

---

## 🔍 验证部署

### 检查服务状态

```bash
# 检查服务是否运行
sudo systemctl status geshixiugai

# 检查端口监听
sudo netstat -tlnp | grep 8000

# 测试本地访问
curl http://localhost:8000
curl http://121.199.49.1
```

### 查看日志

```bash
# 查看服务日志
sudo journalctl -u geshixiugai -f

# 查看 Nginx 日志
sudo tail -f /var/log/nginx/geshixiugai_error.log
sudo tail -f /var/log/nginx/geshixiugai_access.log
```

---

## 🔧 常用命令

### 重启服务

```bash
# 重启应用服务
sudo systemctl restart geshixiugai

# 重启 Nginx
sudo systemctl restart nginx

# 重新加载 Nginx 配置（不中断服务）
sudo systemctl reload nginx
```

### 更新代码

```bash
cd /var/www/geshixiugai
git pull
source venv/bin/activate
pip install -r requirements.txt
sudo systemctl restart geshixiugai
```

### 查看服务信息

```bash
# 查看服务状态
sudo systemctl status geshixiugai

# 查看服务日志（实时）
sudo journalctl -u geshixiugai -f

# 查看最近 50 条日志
sudo journalctl -u geshixiugai -n 50
```

---

## ⚠️ 故障排查

### 问题 1：无法连接服务器

```bash
# 检查服务器是否在线
ping 121.199.49.1

# 检查 SSH 端口是否开放
telnet 121.199.49.1 22
```

### 问题 2：服务无法启动

```bash
# 查看详细错误
sudo journalctl -u geshixiugai -n 100

# 检查 Python 环境
cd /var/www/geshixiugai
source venv/bin/activate
python -c "import backend.app.main"
```

### 问题 3：502 Bad Gateway

```bash
# 检查服务是否运行
sudo systemctl status geshixiugai

# 检查端口监听
sudo netstat -tlnp | grep 8000

# 检查 Nginx 配置
sudo nginx -t
```

### 问题 4：域名无法访问

```bash
# 检查 DNS 解析
nslookup geshixiugai.cn

# 检查服务器安全组（阿里云控制台）
# 确保开放了 80 和 443 端口
```

---

## 📋 部署检查清单

- [ ] 已连接到服务器（`ssh root@121.199.49.1`）
- [ ] 代码已克隆到 `/var/www/geshixiugai`
- [ ] 部署脚本已运行完成
- [ ] `.env` 文件已配置（存储和支付配置）
- [ ] 服务已启动（`sudo systemctl status geshixiugai`）
- [ ] 域名解析已配置（A 记录指向 121.199.49.1）
- [ ] SSL 证书已配置
- [ ] 网站可以正常访问
- [ ] 文件上传功能正常
- [ ] 在网站底部添加了备案号

---

## 🔐 安全建议

1. **修改 SSH 端口**（可选）：
   ```bash
   sudo nano /etc/ssh/sshd_config
   # 修改 Port 22 为其他端口
   sudo systemctl restart sshd
   ```

2. **配置防火墙**：
   ```bash
   sudo ufw allow 22/tcp
   sudo ufw allow 80/tcp
   sudo ufw allow 443/tcp
   sudo ufw enable
   ```

3. **定期更新系统**：
   ```bash
   sudo apt update && sudo apt upgrade -y
   ```

---

## 📞 需要帮助？

- 查看详细文档：`阿里云快速部署指南.md`
- 查看完整指南：`阿里云迁移完整指南.md`
- 查看准备清单：`部署准备清单.md`

---

**部署完成后，你的网站将在 `https://geshixiugai.cn` 正常运行！** 🎉








