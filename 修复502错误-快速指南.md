# 502 Bad Gateway 错误快速修复指南

## 🔍 问题诊断

502 Bad Gateway 通常表示后端服务（Gunicorn）没有正常运行。

## 🚀 快速修复步骤（按顺序执行）

### 步骤 1: 检查服务状态

```bash
sudo systemctl status geshixiugai
```

**如果显示 `inactive` 或 `failed`**，继续下一步。

### 步骤 2: 查看错误日志

```bash
# 查看最近的错误日志（最后 50 行）
sudo journalctl -u geshixiugai -n 50 --no-pager
```

**请将错误日志发给我，以便准确诊断问题。**

### 步骤 3: 检查代码是否有语法错误

```bash
cd /var/www/geshixiugai

# 激活虚拟环境
source venv/bin/activate

# 测试应用是否能正常导入
python -c "from backend.app.main import app; print('✅ 应用导入成功')" 2>&1
```

**如果导入失败，会显示具体错误信息。**

### 步骤 4: 更新代码（如果导入失败）

```bash
cd /var/www/geshixiugai

# 拉取最新代码
git pull origin main

# 重新安装依赖（如果需要）
pip install -r requirements.txt -i https://pypi.tuna.tsinghua.edu.cn/simple
```

### 步骤 5: 重启服务

```bash
# 重启服务
sudo systemctl restart geshixiugai

# 等待 3 秒
sleep 3

# 检查服务状态
sudo systemctl status geshixiugai
```

**应该显示 `active (running)`**

### 步骤 6: 检查端口监听

```bash
# 检查 8000 端口是否被监听
sudo netstat -tlnp | grep 8000

# 或者
sudo ss -tlnp | grep 8000
```

**应该看到类似输出：**
```
tcp        0      0 127.0.0.1:8000          0.0.0.0:*               LISTEN      12345/gunicorn
```

### 步骤 7: 测试本地访问

```bash
# 测试本地访问
curl http://localhost:8000

# 应该返回 JSON 或重定向响应
```

### 步骤 8: 检查 Nginx 配置

```bash
# 检查 Nginx 状态
sudo systemctl status nginx

# 检查 Nginx 配置语法
sudo nginx -t

# 如果配置有问题，重启 Nginx
sudo systemctl restart nginx
```

---

## 🔧 常见错误和解决方案

### 错误 1: ModuleNotFoundError

**症状**: 日志显示 `ModuleNotFoundError: No module named 'xxx'`

**解决**:
```bash
cd /var/www/geshixiugai
source venv/bin/activate
pip install -r requirements.txt -i https://pypi.tuna.tsinghua.edu.cn/simple
sudo systemctl restart geshixiugai
```

### 错误 2: ImportError 或语法错误

**症状**: 日志显示 `ImportError` 或 `SyntaxError`

**解决**:
```bash
cd /var/www/geshixiugai
git pull origin main
source venv/bin/activate
python -c "from backend.app.main import app; print('✅ 应用导入成功')" 2>&1
```

如果导入失败，请将错误信息发给我。

### 错误 3: 端口被占用

**症状**: 服务启动失败，日志显示端口被占用

**解决**:
```bash
# 查找占用 8000 端口的进程
sudo lsof -i :8000

# 或者
sudo netstat -tlnp | grep 8000

# 如果发现其他进程占用，可以：
# 1. 停止占用端口的进程
# 2. 或者修改 gunicorn_config.py 中的端口号
```

### 错误 4: 权限问题

**症状**: 日志显示 `Permission denied`

**解决**:
```bash
# 检查文件权限
ls -la /var/www/geshixiugai

# 确保服务用户有权限访问
sudo chown -R nginx:nginx /var/www/geshixiugai
```

---

## 📋 完整修复流程（一键执行）

```bash
# 1. 进入项目目录
cd /var/www/geshixiugai

# 2. 更新代码
git pull origin main

# 3. 激活虚拟环境
source venv/bin/activate

# 4. 测试应用导入
python -c "from backend.app.main import app; print('✅ 应用导入成功')" 2>&1

# 5. 如果导入成功，重启服务
sudo systemctl restart geshixiugai

# 6. 等待 3 秒
sleep 3

# 7. 检查服务状态
sudo systemctl status geshixiugai

# 8. 测试本地访问
curl http://localhost:8000

# 9. 查看日志（实时）
sudo journalctl -u geshixiugai -f
```

---

## ⚠️ 如果仍然失败

请提供以下信息：

1. **服务状态**：
   ```bash
   sudo systemctl status geshixiugai
   ```

2. **错误日志**：
   ```bash
   sudo journalctl -u geshixiugai -n 100 --no-pager
   ```

3. **应用导入测试结果**：
   ```bash
   cd /var/www/geshixiugai
   source venv/bin/activate
   python -c "from backend.app.main import app; print('✅ 应用导入成功')" 2>&1
   ```

4. **端口监听情况**：
   ```bash
   sudo netstat -tlnp | grep 8000
   ```

---

## ✅ 验证修复成功

修复成功后，应该能够：

1. ✅ 访问 https://www.geshixiugai.cn
2. ✅ 访问 https://www.geshixiugai.cn/web/convert.html
3. ✅ 服务状态显示 `active (running)`
4. ✅ 本地测试 `curl http://localhost:8000` 返回正常响应



