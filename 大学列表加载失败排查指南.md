# 大学列表加载失败排查指南

## 🔍 问题现象

下拉框显示"加载中..."，大学列表无法加载。

## ✅ 排查步骤

### 步骤 1：检查后端服务是否运行

**在服务器上执行**：

```bash
# 检查服务状态
sudo systemctl status geshixiugai

# 应该看到：Active: active (running)
# 如果显示 failed 或 inactive，需要重启服务
```

### 步骤 2：检查预设模板文件是否存在

**在服务器上执行**：

```bash
# 进入项目目录
cd /var/www/geshixiugai

# 检查文件是否存在
ls -la backend/app/services/university_templates.json

# 如果文件不存在，需要从GitHub拉取或手动创建
```

### 步骤 3：如果文件不存在，更新代码

**在服务器上执行**：

```bash
# 确保在项目目录
cd /var/www/geshixiugai

# 更新代码（拉取最新代码，包含模板文件）
git pull origin main

# 如果 git pull 失败，检查文件是否在代码库中
git ls-files | grep university_templates.json
```

### 步骤 4：检查文件内容

**在服务器上执行**：

```bash
# 查看文件前几行
head -20 backend/app/services/university_templates.json

# 检查JSON格式是否正确
python3 -m json.tool backend/app/services/university_templates.json > /dev/null
if [ $? -eq 0 ]; then
  echo "✅ JSON格式正确"
else
  echo "❌ JSON格式错误"
fi
```

### 步骤 5：查看服务日志

**在服务器上执行**：

```bash
# 查看最近的日志
sudo journalctl -u geshixiugai -n 100 --no-pager | grep -i "template\|university"

# 应该看到类似：
# [UniversityTemplateService] 尝试加载模板文件: ...
# [UniversityTemplateService] 文件是否存在: True
# [UniversityTemplateService] 成功加载模板文件，包含 6 所大学
```

### 步骤 6：测试API端点

**在浏览器中访问**：

```
https://你的域名/templates/presets
```

**或者使用 curl**：

```bash
# 在服务器上执行
curl http://localhost:8000/templates/presets

# 应该返回JSON格式的大学列表
```

### 步骤 7：检查前端控制台错误

**在浏览器中**：

1. 按 `F12` 打开开发者工具
2. 切换到 `Console` 标签
3. 查看是否有错误信息
4. 切换到 `Network` 标签
5. 刷新页面，查看 `/templates/presets` 请求的状态

**常见错误**：
- `Failed to fetch` - 后端服务未启动或网络问题
- `404 Not Found` - API路径错误
- `500 Internal Server Error` - 后端代码错误

## 🔧 快速修复

### 如果文件不存在

**在服务器上执行**：

```bash
cd /var/www/geshixiugai

# 方法1：从GitHub拉取最新代码
git pull origin main

# 方法2：如果文件不在代码库，手动创建
cat > backend/app/services/university_templates.json << 'EOF'
{
  "universities": [
    {
      "id": "hdu",
      "name": "杭州电子科技大学",
      "display_name": "杭州电子科技大学",
      "description": "杭州电子科技大学毕业论文格式规范（2026届）"
    }
  ]
}
EOF
```

### 如果服务未启动

**在服务器上执行**：

```bash
# 重启服务
sudo systemctl restart geshixiugai

# 检查状态
sudo systemctl status geshixiugai
```

### 如果API返回空列表

**检查日志**：

```bash
# 查看详细错误
sudo journalctl -u geshixiugai -n 200 --no-pager
```

## 📝 验证修复

修复后，在浏览器中：

1. 刷新页面
2. 打开开发者工具（F12）
3. 查看 `Network` 标签中的 `/templates/presets` 请求
4. 应该返回包含大学列表的JSON
5. 下拉框应该显示大学选项

## 🆘 如果还是不行

1. **检查文件权限**：
   ```bash
   ls -la backend/app/services/university_templates.json
   # 确保文件可读
   ```

2. **检查Python路径**：
   ```bash
   # 在虚拟环境中测试
   cd /var/www/geshixiugai
   source venv/bin/activate
   python -c "from backend.app.services.university_template_service import UniversityTemplateService; s = UniversityTemplateService(); print(s.get_all_universities())"
   ```

3. **查看完整错误日志**：
   ```bash
   sudo journalctl -u geshixiugai -n 500 --no-pager
   ```





